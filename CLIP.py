...
class CLIP(nn.Module):
    def __init__(self, patch_number=49, patch_size=32, proj_size=512, num_hidden_layers=12, visual_hidden_size=768, text_hidden_size=512, visual_num_attention_heads=12, text_num_attention_heads=8, attention_probs_dropout_prob=0.1, hidden_dropout_prob=0.1):
        ...
        # NOTE: token embedding is a lookup table that map the token index to a 512-D
        # vector. Preliminary examination shows that this embedding table seems to
        # be trained along side with the text model
    ...
    def encode_text(self, input):
        ...
        seq_end = torch.argmax(input, dim=1, keepdim=True) # get the index of end-of-sequence

        grid_ind, grid_pos = torch.meshgrid(
            torch.arange(seq_end.size(0), dtype=torch.long, device=token_emb.device),
